...
float16_t* lattice_model::update_posterior_probability(int state, int response, float** dilution_matrix){
    int size = (1 << pool_size) / _v_size;
    float16_t* ret = float16_alloc(size);
    float16_t denominator = f16zero;
    if(response == 1){
        for(int i = 0; i < (1 << pool_size); i+=_v_size){
...
float** lattice_model::generate_dilution_matrix(float alpha, float h){
    float** ret = new float*[pool_size];
    int k;
    for(int rk = 1; rk <= pool_size; rk++){
        ret[rk-1] = new float[rk+1];
        ret[rk-1][0] = alpha;
        for(int r = 1; r <= rk; r++){
            k = rk-r;
...
float16_t* lattice_model::compute_dilution_response(float** dilution_matrix, int e, int state){
    float16_t* ret = float16_alloc(1);
    for(int i = start_state; i < _v_size; i++){
        int index = __builtin_popcount(i);
        ret[0][i] = dilution_matrix[index-1][index-__builtin_popcount(e & i)];
    }
}

int lattice_model::find_halving_state(float prob){
...
int lattice_model::find_halving_state_openmp_static_scheduler(float prob){
    int ret = 0, size = (1 << pool_size);
    float thres = 2.0;
    std::vector<bool> checked_set(size);
    int* sophisticated_selection_table = sophisticated_selection();
    ...
    for(int i = 0; i < size; i++){
        int state = sophisticated_selection_table[i];
        if(checked_set[state]) continue;
        float val = get_up_set_mass(state);
        if(val < prob){
            int* up_set = get_up_set(state);
            for(int j = 0; j < (1 << (pool_size-__builtin_popcount(state))); j++) checked_set[up_set[j]] = true;
            delete[] up_set;
...
int lattice_model::find_halving_state_openmp_dynamic_scheduler(float prob){
    int ret = 0, size = (1 << pool_size);
    float thres = 2.0;
    std::vector<bool> checked_set(size);
    int* sophisticated_selection_table = sophisticated_selection();
...
float lattice_model::get_up_set_mass(int state){
